...
from geonode.utils import json_response2
        ...
        for srv, mlist in _metrics:
            out.append({'service': srv.name,
                        'metrics': [{'name': m.name, 'unit': m.unit, 'type': m.type}
                                    for m in mlist]})
        return json_response2({'metrics': out})
...
class ServicesList(View):
    ...
    def get(self, *args, **kwargs):
        ...
        return json_response2({'services': out})
...
class HostsList(View):
    ...
    def get(self, *args, **kwargs):
        ...
        return json_response2({'hosts': out})
...
class FilteredView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.errors:
            return json_response2({'success': False,
                                  'status': 'errors',
                                  'errors': self.errors},
                                 status=400)
...
class MetricDataView(View):
    ...
    def get(self, *args, **kwargs):
        ...
        if self.errors:
            return json_response2({'status': 'error',
                                  'success': False,
                                  'errors': self.errors},
                                 status=400)
        ...
        return json_response2({'data': out})
...
class BeaconView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not service:
            ...
            return json_response2({'exposed': data})
        try:
            ex = exposes[service]()
        except KeyError:
            return json_response2(
...
class UserNotificationConfigView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.user.is_authenticated():
            ...
            status = 401
        return json_response2(out, status=status)
...
class AutoconfigureView(View):
    def post(self, request, *args, **kwargs):
        if not request.user.is_authenticated():
            ...
            return json_response2(out, status=401)
